...
static void can_write(void *arg, long period) {
    ...
    if (first_run) {
        uint32_t start_id = make_can_id(PRIO_HIGH, NODE_CONTROLLER, NODE_REMORA, MSG_START, 0);
        can_send(start_id, NULL, 0);  // No payload needed
        first_run = false;
    }

    static bool last_reset = false;
    if (*(data->CANreset) && !last_reset) {
        // Rising edge of reset - send STOP then START
        uint32_t stop_id = make_can_id(PRIO_HIGH, NODE_CONTROLLER, NODE_REMORA, MSG_STOP, 0);
        can_send(stop_id, NULL, 0);

        // Brief delay to allow stop to process
        usleep(1000);  // 1ms delay

        uint32_t start_id = make_can_id(PRIO_HIGH, NODE_CONTROLLER, NODE_REMORA, MSG_START, 0);
        can_send(start_id, NULL, 0);
    }
    last_reset = *(data->CANreset);

    // Only send other messages if we have status
    if (!*(data->CANstatus)) {
        return;
    }
    ...
    for (int i = 0; i < JOINTS; i++) {
        can_id = make_can_id(PRIO_MEDIUM, NODE_CONTROLLER, NODE_REMORA, MSG_JOINT_CMD, i);
    ...
    for (int i = 0; i < VARIABLES; i++) {
        can_id = make_can_id(PRIO_MEDIUM, NODE_CONTROLLER, NODE_REMORA, MSG_SETPOINT, i);
    ...
    for (int i = 0; i < (DIGITAL_OUTPUTS + 7) / 8; i++) {
        can_id = make_can_id(PRIO_LOW, NODE_CONTROLLER, NODE_REMORA, MSG_DIG_OUT, i);
        uint8_t out_byte = 0;
    ...
    // Send enables status
    can_id = make_can_id(PRIO_HIGH, NODE_CONTROLLER, NODE_REMORA, MSG_JOINT_ENABLE, 0);
    can_send(can_id, &joint_enables, 1);
}
