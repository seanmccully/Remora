...
unique_ptr<Module> createTMC2209(const JsonObject& config) {
    ...
    return make_unique<TMC2209>(RxPin, RSense, address, current, microsteps, stealthchop, stall);
}
...
************************************************************************/
    ...
    // TMC2209(const char*, float, uint8_t, uint16_t, uint16_t, bool, uint16_t);
TMC2209::TMC2209(const char* rxtxPin, float Rsense, uint8_t addr, uint16_t mA, uint16_t microsteps, bool stealth, uint16_t stall) :
    rxtxPin(rxtxPin),
    mA(mA),
